...
| LLM Provider | Gemini API | embedding、tagging、chat response |
...
- PostgreSQL
- pgvector
- Google Gen AI SDK 或 Gemini REST API
...
- Gemini API key 透過 environment variables 管理
...
> 開發 AI 工程筆記系統，使用 React、FastAPI、PostgreSQL/pgvector 與 Gemini API 建立個人技術知識庫。系統支援 Markdown 筆記管理、AI 自動標籤、語意搜尋與 RAG 問答，並透過 Docker Compose 完成本機部署環境。專案實作 embedding pipeline、chunking strategy、vector similarity search 與 LLM context injection，用於提升工程知識檢索與技術筆記整理效率。
